...
export default function App() {
  ...
  useEffect(() => {
    ...
    const loadPosts = async () => {
      try {
        const { posts: apiPosts } = await apiGetPosts("default");
  ...
  const handleSavePost = async (e: React.FormEvent) => {
    ...
    if (editingPost) {
      ...
      try {
        const { post: created } = await apiCreatePost("default", {
          title: formTitle,
          content: formContent,
          platforms: formPlatforms,
          status: "draft",
    ...
    for (const postId of selectedPostIds) {
      ...
      try {
        ...
        if (result.success) {
          ...
          results.push(`✗ ${post.title}: ${result.results.map((r: any) => r.error).join(", ")}`);
        }
      } catch {
        setPosts((prev) => prev.map((p) => p.id === postId ? { ...p, status: post.status } : p));
        results.push(`✗ ${post.title}: API error`);
      }
    }
                  ...
                  <tr key={post.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-1 rounded-full ${
                        ...
                        post.status === 'scheduled' ? 'bg-indigo-50 text-indigo-700' :
                    ...
                    <td className="px-4 py-3">
                      {post.attachments.length > 0 ? (
                        <div className="flex items-center gap-1 text-xs text-slate-600">
                          {post.attachments[0].type.startsWith('image') ? <Image className="w-4 h-4" /> : <Video className="w-4 h-4" />}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {["facebook", "instagram", "twitter", "linkedin", "tiktok", "youtube"].map(platform => (
                    <label key={platform} className="flex items-center gap-2 px-3 py-2 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50">
                      <input
                        type="checkbox"
              ...
              <div>
                ...
                <div className="space-y-2">
                  {formAttachments.map(att => (
                    <div key={att.id} className="flex items-center justify-between p-2 bg-slate-50 rounded-lg">
                      <div className="flex items-center gap-2">
                        {att.type.startsWith('image') ? <Image className="w-4 h-4 text-slate-400" /> : <Video className="w-4 h-4 text-slate-400" />}
                  ...
                  <button
                    ...
                    className="w-full px-3 py-2 border-2 border-dashed border-slate-200 rounded-lg text-sm text-slate-600 hover:border-indigo-400 hover:text-indigo-600"
                  >
                    + Add Media
                  </button>
                </div>
